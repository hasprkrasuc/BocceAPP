...
/** Možne točke = odigrano × 2 (vsaka disciplina prinese 0/1/2). */
const possiblePts = (played: number) => played * 2
/** Uspešnost = osvojene / možne točke, v %. */
const pctStr = (matchPointsFor: number, played: number) =>
  played > 0 ? `${Math.round((matchPointsFor / (played * 2)) * 100)} %` : '–'
      ...
      {view === 'player' && (
        <table className="w-full text-sm">
          <thead><tr className="text-left text-gray-500 border-b">
            ...
            <th className="text-right">Možne točke</th><th className="text-right">Točke</th><th className="text-right">Uspešnost</th>
          </tr></thead>
          <tbody>
            {playerStats.map((p, i) => (
              <tr key={p.playerId} className="border-b">
                ...
                <td className="text-right">{possiblePts(p.totalPlayed)}</td>
                <td className="text-right font-semibold">{p.totalMatchPointsFor}</td>
                <td className="text-right">{pctStr(p.totalMatchPointsFor, p.totalPlayed)}</td>
              </tr>
            ))}
            {playerStats.length === 0 && <tr><td colSpan={6} className="py-2 text-gray-400">Ni podatkov.</td></tr>}
          </tbody>
        </table>
      )}
      ...
      {view === 'playerDisc' && (() => {
        ...
        return (
          <div>
            ...
            <table className="w-full text-sm">
              <thead><tr className="text-left text-gray-500 border-b">
                <th className="py-1">Igralec</th><th className="text-right">Možne točke</th>
                <th className="text-right">Točke</th><th className="text-right">Uspešnost</th>
              ...
              <tbody>
                {g.rows.map(r => (
                  <tr key={r.playerId} className="border-b">
                    ...
                    <td className="text-right">{possiblePts(r.played)}</td>
                    <td className="text-right font-semibold">{r.matchPointsFor}</td>
                    <td className="text-right">{pctStr(r.matchPointsFor, r.played)}</td>
      ...
      {view === 'teamDisc' && (() => {
        ...
        return (
          <div>
            ...
            <table className="w-full text-sm">
              <thead><tr className="text-left text-gray-500 border-b">
                <th className="py-1">Ekipa</th><th className="text-right">Možne točke</th>
                <th className="text-right">Točke</th><th className="text-right">Uspešnost</th>
                {showsAverage(g.type) && <th className="text-right">Povprečje</th>}
              </tr></thead>
              <tbody>
