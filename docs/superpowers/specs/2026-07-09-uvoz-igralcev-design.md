...
Standardni obrazec (ena datoteka = en klub). Struktura (potrjeno na realnem primeru registracijskega Excela kluba):
...
- **Klub** se bere iz **glave** ("Balinarski klub"), NE iz stolpca "Klub" v tabeli — ta ima v podatkih tipkarske napake (npr. "BK PRIMERR" proti "BK PRIMER").
...
- **Parser (enote):** glava (klub, sezona), obe obliki datuma, EMŠO validacija, konec tabele ob nogi, ignoriranje "Vodje ekipe". Testni vzorec: registracijski Excel realnega kluba (20 igralcev, oba formata datuma, tipkarski klub v stolpcu).
...
- **E2E (ročno):** naloži testni registracijski Excel na testni ekipi → predogled → uvoz → preveri /klubi (člani) in ligaški roster.
